...
lines(x.pred.is[2:slut.is] + DK1$s.pred[2:slut.is], col = "red")
...
res.oos <- OOS$D - x.pred.oos[start.oos:slut.oos]

res.is <- DK1$D[2:slut.is] - x.pred.is[2:slut.is]

save(x.pred.is, x.pred.oos, rmse.is, rmse.oos, a1, a3, p, mu2, pred.inter, res.is, res.oos,
