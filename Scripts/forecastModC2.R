...
rmse.is.c <- sqrt(1/slut.is * sum((dat[2:slut.is] - x.pred.is.c)^2))
rmse.is.c

mae.is  <- mean(abs(dat[2:slut.is] - x.pred.is.c))
mae.is
...
for (l in start.oos:slut.oos) {
  ...
  theta0 <- c(sigma1, sigma2, sigma2, alpha1, alpha3, mu2, beta[1], beta[2])
