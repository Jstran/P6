...
modEscClean <- nls(Obs ~ B0 + B1 * t + B2 * t^2 + C1 * sin((C2 + t) * 2*pi/365.25) + C3 * 
                     sin((C4 + t) * 4*pi/365.25) + D1 * sat + D2 * sun,
                   start = c(B0 = 1, B1 = 1 , B2 = 1, C1 = 1, C2 = 1, C3 = 1, C4 = 1, D1 = 1, D2 = 1), data = dfClean)
...
lmEsc <- lm(DK1$Clean ~ t + I(t^2) + 
              sin((2*pi/365.25)*t) + cos((2*pi/365.25)*t) + 
              sin((4*pi/365.25)*t) + cos((4*pi/365.25)*t) + 
              DK1$sat + DK1$sun) ; summary(lmEsc)

pf <- data.frame(dates = datesY ,
                 price = DK1$Clean ,
                 pred  = predict(lmEsc) ,
                 predShift = predict(lmEsc) - 200 ,
                 priceShift = DK1$Clean - predict(lmEsc))


pt <- ggplot() +
  geom_line( aes(x = pf$dates, y = pf$price ,      col = "Price"), colour = colors[1])       +
  geom_line( aes(x = pf$dates, y = pf$priceShift , col = "Price Shift"), colour = colors[2]) +
  geom_point(aes(x = pf$dates, y = pf$pred,        col = "Pred"), colour = colors[3])        +
  geom_point(aes(x = pf$dates, y = pf$predShift ,  col = "Pred Shift"), colour = colors[4])  +
